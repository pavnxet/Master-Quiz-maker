/**
 * Cloudflare Workers — Quiz Generator + Telegram Bot + Turso DB
 * ──────────────────────────────────────────────────────────────
...
async function dbExec(env, statements) {
  ...
  try {
    const r = await fetch(`${env.TURSO_DB_URL}/v2/pipeline`, {
      method: 'POST',
      headers: {
    ...
    if (!r.ok) { console.error('Turso HTTP error', r.status); return null; }
    return r.json();
  } catch (e) { console.error('Turso fetch error:', e.message); return null; }
}
...
async function trackGeneration(env, { source, title, questionsCount, chatId, username, firstName }) {
  if (!hasDb(env)) return;
  try {
    await dbExec(env, [
      {
  ...
  } catch (e) { console.error('trackGeneration error:', e.message); }
}
  ...
  <div class="card">
    ...
    <div class="quiz-nav">
      <div class="quiz-nav-left">
        ...
        <button class="flag-btn" id="flag-btn" onclick="toggleFlag()" title="Flag for review (*)">⭐</button>
      </div>
...
<div id="results-page" class="page">
  ...
  <div style="display:flex;gap:9px;flex-wrap:wrap;margin-bottom:18px">
    <button class="btn btn-primary"  onclick="showPage('review-page')">🔍 Review</button>
    <button class="btn btn-outline"  id="retake-btn" onclick="retakeQuiz()">🔁 Retake</button>
    <button class="btn btn-outline"  onclick="showPage('quiz-setup')">🔄 New Quiz</button>
    <button class="btn btn-success"  onclick="copyResults()">📋 Copy Results</button>
    <button class="btn btn-outline"  onclick="window.print()">🖨️ Print Review</button>
...
<div id="stats-page" class="page">
  ...
  <div class="card">
    ...
    <button class="btn btn-sm btn-outline"
      style="margin-top:9px;border-color:var(--danger);color:var(--danger)"
...
function clearHistory(){if(!confirm('Clear all history?'))return;quizHistory=[];saveHistory();renderStatsPage();renderHomePage();}
...
function populateFilters(){
  const subSel=document.getElementById('filter-subject'),revSub=document.getElementById('review-subject');
  getSubjects().forEach(s=>{[subSel,revSub].forEach(el=>{const o=document.createElement('option');o.value=s;o.textContent=s;el.appendChild(o);});});
  subSel.addEventListener('change',()=>{
    const topSel=document.getElementById('filter-topic');topSel.innerHTML='<option value="">All Topics</option>';
...
function startQuiz(){
  ...
  qs=qs.slice(0,cnt);
  // Scramble options: shuffle optionsEnglish/Hindi together and remap correct index
  if(scramble){
    qs=qs.map(q=>{
...
function buildQuizNav(){
  ...
  currentSession.questions.forEach((_,i)=>{
    const d=document.createElement('div');d.className='q-dot';d.textContent=i+1;
    d.onclick=()=>jumpTo(i);grid.appendChild(d);
  });
}
...
function renderQuestion(){
  ...
  else if(isLast){skipBtn.textContent='✓ Finish';skipBtn.onclick=finishQuiz;skipBtn.style.display='';}
  else{skipBtn.textContent='Skip';skipBtn.onclick=skipQuestion;skipBtn.style.display='';}
  if(s.secPerQ!==null){s.timeLeft=s.secPerQ;updateTimerDisplay();}
  updateNavDots();updateFlagBtn();updateFlagCountLabel();
}
function selectOption(i){
  const s=currentSession;if(s.revealed[s.currentIdx])return;
  s.answers[s.currentIdx]=i;s.revealed[s.currentIdx]=true;
  renderQuestion();
...
function skipQuestion(){
  const s=currentSession;if(s.answers[s.currentIdx]===null)s.answers[s.currentIdx]=-1;
  goQuestion(1);
}
...
function finishQuiz(){
  ...
  const result={
    date:new Date().toISOString(),total:s.questions.length,correct,wrong,skipped,pct,
  ...
  quizHistory.unshift(result);if(quizHistory.length>50)quizHistory=quizHistory.slice(0,50);
  saveHistory();currentSession._lastResult=result;showPage('results-page');
}
...
function copyResults(){
  ...
  navigator.clipboard.writeText(lines.join('\\n')).then(()=>showToast('✅ Copied to clipboard!')).catch(()=>showToast('❌ Copy failed'));
}
...
function renderReview(){
  ...
  }else{ALL_QUESTIONS.forEach(q=>items.push({q,a:null,status:'unanswered',flagged:false}));}
...
function renderStatsPage(){
  if(!quizHistory.length){
    ...
    document.getElementById('stats-history').innerHTML='';return;
  }
...
/* Drop zone — clicking anywhere in it opens the file picker */
...
/* The label is the ONLY real click target for the file input */
...
<body>
<div class="card">
  <button class="dark-toggle" id="dk" onclick="toggleDark()" title="Toggle dark mode">🌙</button>
  <h1>🎓 Quiz Generator</h1>
  <p class="sub">Upload one or more JSON quiz files — merge &amp; generate an interactive bilingual HTML quiz.</p>
  <div class="tg-banner">
    📱 <strong>Telegram Bot:</strong> Send your <code>.json</code> file directly to the bot and get the HTML quiz instantly. Use <code>/mystats</code> to see your history.
  </div>

  <!-- Hidden file input — OUTSIDE drop zone to avoid label double-trigger -->
  ...
  <button class="btn" id="sb" disabled onclick="generate()">⬇️ Generate &amp; Download Quiz HTML</button>
...
<script>
// Dark mode
function applyDark(on){document.body.classList.toggle('dark',on);document.getElementById('dk').textContent=on?'☀️':'🌙';localStorage.setItem('qg_theme',on?'dark':'');}
function toggleDark(){applyDark(!document.body.classList.contains('dark'));}
(function initDark(){const s=localStorage.getItem('qg_theme');if(s==='dark')applyDark(true);else if(!s&&window.matchMedia('(prefers-color-scheme:dark)').matches)applyDark(true);})();
...
// Clicking anywhere on the drop zone (except the label, which handles itself)
...
function renderList(){
  fl.innerHTML=selectedFiles.map((f,i)=>
    ...
    ' <button onclick="removeFile('+i+')">✕</button></span>'
  ).join('');
  sb.disabled=!selectedFiles.length;
  mn.style.display=selectedFiles.length>1?'block':'none';
}
function removeFile(i){const f=selectedFiles.splice(i,1)[0];delete fileCounts[f.name];renderList();}
...
Send me a <code>.json</code> file and I'll return a fully interactive bilingual HTML quiz.
...
async function handleTelegram(request, env) {
  ...
  if (text.startsWith('/mystats')) {
    ...
    if (!stats) {
      await tgSend(token, chatId, '📊 No stats yet — send a <code>.json</code> file to generate your first quiz!', {parse_mode:'HTML'});
    } else {
  ...
  if (text.startsWith('/globalstats')) {
    ...
    if (!stats) {
      await tgSend(token, chatId, '📊 No data yet.');
    } else {
      await tgSend(token, chatId,
        `🌍 <b>Platform Stats</b>\n\n` +
  ...
  if (doc) {
    ...
    if (!fileUrl) {
      await tgSend(token, chatId, '❌ Could not access your file. Please try again.');
      return okResp();
    }
    let content;
    try { content = await (await fetch(fileUrl)).text(); } catch {
      await tgSend(token, chatId, '❌ Failed to download the file. Please try again.');
      return okResp();
    }
    let questions;
    ...
    if (!result?.ok) {
      await tgSend(token, chatId, `⚠️ Could not send file: ${escHtml(result?.description||'unknown error')}`);
    }
    // Track in Turso DB (non-blocking)
    trackGeneration(env, {
      source: 'telegram', title, questionsCount: questions.length,
...
// ════════════════════════════════════════════════════════════
//  WEBHOOK SETUP  (GET /setup — run once)
// ════════════════════════════════════════════════════════════
...
async function handleGenerate(request, env) {
  ...
  const htmlOut = generateHtml(questions, title);
  const outName = filename.endsWith('.html')
    ? filename
    : filename.replace(/\.\w+$/,'') + '_quiz.html';

  // Track in Turso DB (non-blocking — don't await)
...
// ════════════════════════════════════════════════════════════
//  WEB: GET /dbstats  — aggregate stats JSON
// ════════════════════════════════════════════════════════════
async function handleDbStats(env) {
  const stats = await getDbStats(env);
  if (!stats) return jsonResponse({error:'Database not configured or no data.'}, hasDb(env)?200:503);
...
export default {
  async fetch(request, env) {
    ...
    // DB init (run once after first deploy with Turso)
    if (method === 'GET' && path === '/initdb') {
      if (!hasDb(env))
        return jsonResponse({error:'TURSO_DB_URL and TURSO_AUTH_TOKEN not set.'}, 400);
      await initDb(env);
    ...
    // DB stats for upload page
    if (method === 'GET' && path === '/dbstats')
      return handleDbStats(env);
