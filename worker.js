/**
 * Cloudflare Workers — Quiz Generator + Telegram Bot
 * ───────────────────────────────────────────────────
 * Web interface:  GET  /          → drag-and-drop upload UI
 *                 POST /generate  → returns self-contained HTML quiz
 * Telegram bot:   POST /telegram  → webhook receiver (set TELEGRAM_TOKEN)
 * One-time setup: GET  /setup     → registers Telegram webhook
 *
 * Environment variables (Cloudflare dashboard → Worker → Settings → Variables):
 *   TELEGRAM_TOKEN   e.g.  123456:ABC-DEF...  (from @BotFather)
 */

// ════════════════════════════════════════════════════════════
//  QUIZ HTML GENERATOR  (self-contained bilingual output)
// ════════════════════════════════════════════════════════════
  ...
  <div class="card">
    ...
    <div class="filter-row">
      ...
      <select id="review-filter" onchange="renderReview()">
        <option value="all">All</option><option value="wrong">Wrong</option>
        <option value="correct">Correct</option><option value="skipped">Skipped</option>
      </select>
    </div>
  </div>
...
titleEl.addEventListener('input', () => {
  heroEl.textContent = titleEl.textContent.trim() || '${titleSafe}';
  document.title = titleEl.textContent.trim() || '${titleSafe}';
});
titleEl.addEventListener('blur', () => {
  if (!titleEl.textContent.trim()) titleEl.textContent = '${titleSafe}';
...
function showPage(id) {
  ...
  const m = {home:0,'quiz-setup':1,'quiz-page':1,'results-page':1,'review-page':2,'stats-page':3};
  document.querySelectorAll('.nav-tab').forEach((t,i) => t.classList.toggle('active', i === m[id]));
...
function populateFilters() {
  ...
  updateAvailable(); updateMarkingPreview();
}
function updateAvailable() {
  const sub=document.getElementById('filter-subject').value, top=document.getElementById('filter-topic').value;
...
function updateMarkingPreview() {
  ...
  document.getElementById('marking-preview').textContent='Correct = +'+mc+' mark(s)  ·  Wrong = -'+mn+' mark(s)  ·  Skip = 0';
}
...
function startQuiz() {
  ...
  let qs=[...pool]; if(ord==='random') qs=qs.sort(()=>Math.random()-.5); qs=qs.slice(0,cnt);
  const secPerQ=mode==='timed'?60:mode==='custom'?cust:null;
  currentSession={questions:qs,answers:new Array(qs.length).fill(null),revealed:new Array(qs.length).fill(false),
    currentIdx:0,startTime:Date.now(),secPerQ,timeLeft:secPerQ,timerInterval:null,
    subject:sub||'All',topic:top||'All',markCorrect:mc,markNeg:mn};
  buildQuizNav(); showPage('quiz-page'); renderQuestion(); if(secPerQ!==null) startTimer();
}

// Quiz engine
function buildQuizNav() {
  const grid=document.getElementById('q-nav-grid'); grid.innerHTML='';
  currentSession.questions.forEach((_,i)=>{ const d=document.createElement('div'); d.className='q-dot'; d.textContent=i+1; d.onclick=()=>jumpTo(i); grid.appendChild(d); });
}
function updateNavDots() {
  document.querySelectorAll('.q-dot').forEach((d,i)=>{ const s=currentSession; d.className='q-dot';
...
function renderQuestion() {
  ...
  document.getElementById('quiz-marking-badge').innerHTML='<span class="pos">+'+s.markCorrect+' correct</span>'+(s.markNeg>0?'<span class="neg">-'+s.markNeg+' wrong</span>':'<span>No negative</span>');
  ...
  optsEn.forEach((opt,i)=>{
    ...
    if(revealed){li.classList.add('disabled');if(i===q.correct)li.classList.add('correct');else if(i===chosen&&chosen!==q.correct)li.classList.add('wrong');}
    else if(i===chosen)li.classList.add('selected');
    const hiOpt=optsHi[i]||'';
    li.innerHTML='<span class="option-letter">'+String.fromCharCode(65+i)+'</span><span class="option-texts"><span class="opt-english">'+opt+'</span>'+(hiOpt?'<span class="opt-hindi">'+hiOpt+'</span>':'')+'</span>';
  ...
  if(revealed){
    ...
    const expHi=document.getElementById('exp-hindi'); expHi.textContent=q.explanationHindi||''; expHi.style.display=q.explanationHindi?'':'none';
  ...
  const nb=document.getElementById('next-btn'); nb.textContent=isLast?'✓ Finish':'Next →'; nb.onclick=isLast?finishQuiz:()=>goQuestion(1);
...
function selectOption(i){
  ...
  s.answers[s.currentIdx]=i; s.revealed[s.currentIdx]=true; renderQuestion();
...
function goQuestion(dir){
  ...
  s.currentIdx=next; if(s.secPerQ!==null){clearInterval(s.timerInterval);s.timeLeft=s.secPerQ;startTimer();}
  renderQuestion();
}
...
function updateTimerDisplay(){
  ...
  if(s.secPerQ===null){el.style.display='none';return;} el.style.display='';
  const t=s.timeLeft; el.textContent=Math.floor(t/60)+':'+String(t%60).padStart(2,'0');
...
function finishQuiz(){
  ...
  let correct=0,wrong=0,skipped=0,totalScore=0; const subjectStats={};
  ...
  const result={date:new Date().toISOString(),total:s.questions.length,correct,wrong,skipped,pct,
    totalScore,maxScore,markCorrect:s.markCorrect,markNeg:s.markNeg,elapsed,
    subject:s.subject,topic:s.topic,subjectStats,answers:[...s.answers],
    questions:s.questions.map(q=>({qEnglish:q.qEnglish,qHindi:q.qHindi,optionsEnglish:q.optionsEnglish,
      optionsHindi:q.optionsHindi,correct:q.correct,explanationEnglish:q.explanationEnglish,
      explanationHindi:q.explanationHindi,subject:q.subject,topic:q.topic}))};
  history.unshift(result); if(history.length>50)history=history.slice(0,50); saveHistory();
  currentSession._lastResult=result; showPage('results-page');
}
function renderResults(){
  if(!currentSession?._lastResult)return; const r=currentSession._lastResult;
  ...
  document.getElementById('marks-display').innerHTML='Score: <span>'+r.totalScore+' / '+r.maxScore+'</span> marks &nbsp;·&nbsp; +'+r.markCorrect+' per correct'+(r.markNeg>0?', -'+r.markNeg+' per wrong':'');
  ...
  Object.entries(r.subjectStats).forEach(([sub,st])=>{const p=Math.round(st.correct/st.total*100);sb.innerHTML+='<div class="sub-row"><span class="sub-name">'+sub+'</span><span class="sub-pct">'+p+'%</span><span class="sub-total">'+st.total+'</span></div>';});
}
...
function renderReview(){
  ...
  let items=[];
  if(currentSession?._lastResult){const r=currentSession._lastResult;r.questions.forEach((q,i)=>{const a=r.answers[i];const status=(a===-1||a===null)?'skipped':a===q.correct?'correct':'wrong';items.push({q,a,status});});}
  else{ALL_QUESTIONS.forEach(q=>items.push({q,a:null,status:'unanswered'}));}
  ...
  list.innerHTML=items.length?items.map((it,n)=>reviewCard(it,n+1)).join(''):'<div class="empty-state"><div class="icon">🔍</div><p>No questions.</p></div>';
}
function reviewCard({q,a,status},n){
  const optsEn=q.optionsEnglish||[],optsHi=q.optionsHindi||[];
  const optRows=optsEn.map((opt,i)=>{
    let cls='';if(i===q.correct)cls='correct-opt';else if(a!==null&&a!==-1&&i===a&&a!==q.correct)cls='user-wrong';
    const icon=i===q.correct?'✓':(a!==null&&a!==-1&&i===a)?'✗':'○';
    const hiOpt=optsHi[i]||'';
    return '<div class="review-option '+cls+'"><span style="font-weight:700;flex-shrink:0;width:22px">'+icon+String.fromCharCode(65+i)+'</span><span class="option-texts"><span class="opt-english">'+opt+'</span>'+(hiOpt?'<span class="opt-hindi">'+hiOpt+'</span>':'')+'</span></div>';
  }).join('');
  ...
  return '<div class="review-q"><div class="q-number">Q'+n+' — '+subTop+' <span class="badge '+badgeCls+'">'+badgeTxt+'</span></div><div class="q-english">'+(q.qEnglish||'')+'</div>'+(q.qHindi?'<div class="q-hindi">'+q.qHindi+'</div>':'')+optRows+'<div class="review-exp-wrap">'+(q.explanationEnglish?'<div class="review-exp-en">💡 '+q.explanationEnglish+'</div>':'')+( q.explanationHindi?'<div class="review-exp-hi">💡 '+q.explanationHindi+'</div>':'')+'</div></div>';
}

// Stats
function renderStatsPage(){
  if(!history.length){document.getElementById('stats-grid').innerHTML='';document.getElementById('stats-subject').innerHTML='<div class="empty-state"><div class="icon">📊</div><p>Take a quiz first!</p></div>';document.getElementById('stats-history').innerHTML='';return;}
  ...
  document.getElementById('stats-grid').innerHTML='<div class="stat-card"><div class="value">'+history.length+'</div><div class="label">Sessions</div></div><div class="stat-card"><div class="value">'+totalQ+'</div><div class="label">Attempted</div></div><div class="stat-card"><div class="value">'+avgPct+'%</div><div class="label">Avg Accuracy</div></div><div class="stat-card"><div class="value">'+best+'%</div><div class="label">Best Score</div></div><div class="stat-card"><div class="value">'+bestMk+'</div><div class="label">Best Marks</div></div>';
  const subAcc={};
  history.forEach(r=>Object.entries(r.subjectStats||{}).forEach(([sub,st])=>{if(!subAcc[sub])subAcc[sub]={correct:0,total:0};subAcc[sub].correct+=st.correct;subAcc[sub].total+=st.total;}));
  ...
  Object.entries(subAcc).sort((a,b)=>b[1].total-a[1].total).forEach(([sub,st])=>{const p=Math.round(st.correct/st.total*100);subEl.innerHTML+='<div class="sub-row"><span class="sub-name">'+sub+'</span><span class="sub-pct">'+p+'%</span><span class="sub-total">'+st.total+'</span></div>';});
  document.getElementById('stats-history').innerHTML=history.slice(0,20).map(r=>{
    const d=new Date(r.date),ds=d.toLocaleDateString()+' '+d.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});
    const color=r.pct>=80?'#16a34a':r.pct>=60?'#4f46e5':'#dc2626';
    const mk=r.totalScore!==undefined?' · '+r.totalScore+'/'+r.maxScore+' marks':'';
    return '<div class="history-item"><span class="h-score" style="color:'+color+'">'+r.pct+'%</span><span class="h-detail">'+r.correct+'/'+r.total+mk+' · '+r.subject+'</span><span class="h-date">'+ds+'</span></div>';
  }).join('');
}
...
function renderHomePage(){
  ...
  document.getElementById('home-stats').innerHTML='<div class="stat-card"><div class="value">'+ALL_QUESTIONS.length+'</div><div class="label">Total Questions</div></div><div class="stat-card"><div class="value">'+getSubjects().length+'</div><div class="label">Subjects</div></div><div class="stat-card"><div class="value">'+history.length+'</div><div class="label">Sessions</div></div>'+(best!==null?'<div class="stat-card"><div class="value">'+best+'%</div><div class="label">Best Score</div></div>':'')+(avg!==null?'<div class="stat-card"><div class="value">'+avg+'%</div><div class="label">Avg Score</div></div>':'');
  const ov=document.getElementById('home-overview');
  if(!history.length){ov.innerHTML='<div class="empty-state"><div class="icon">🎯</div><p>No sessions yet. Click <strong>Quiz</strong> to start!</p></div>';}
  else{const last=history[0];const color=last.pct>=80?'#16a34a':last.pct>=60?'#4f46e5':'#dc2626';const mk=last.totalScore!==undefined?' · <strong>'+last.totalScore+'/'+last.maxScore+' marks</strong>':'';ov.innerHTML='<p style="font-size:.86rem;color:var(--muted);margin-bottom:10px">Last: <strong style="color:'+color+'">'+last.pct+'%</strong>'+mk+' — '+last.correct+'/'+last.total+' correct</p>'+barRow('Correct',last.correct,last.total,'#16a34a')+barRow('Wrong',last.wrong,last.total,'#dc2626')+barRow('Skipped',last.skipped,last.total,'#d97706');}
  document.getElementById('home-recent').innerHTML=history.length?history.slice(0,5).map(r=>{const color=r.pct>=80?'#16a34a':r.pct>=60?'#4f46e5':'#dc2626';const mk=r.totalScore!==undefined?' · '+r.totalScore+' marks':'';return '<div class="history-item"><span class="h-score" style="color:'+color+'">'+r.pct+'%</span><span class="h-detail">'+r.correct+'/'+r.total+mk+' · '+r.subject+'</span><span class="h-date">'+new Date(r.date).toLocaleDateString()+'</span></div>';}).join(''):'<div style="color:var(--muted);font-size:.86rem">No sessions yet.</div>';
}
...
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:'Segoe UI',system-ui,sans-serif;background:var(--bg);color:var(--t);min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:20px}
.card{background:var(--s);border:1px solid var(--b);border-radius:16px;box-shadow:0 4px 24px rgba(0,0,0,.08);padding:36px;max-width:500px;width:100%}
h1{color:var(--p);font-size:1.6rem;margin-bottom:5px}
.sub{color:var(--m);font-size:.88rem;margin-bottom:24px}
.tg-banner{background:#e7f3ff;border:1px solid #93c5fd;border-radius:10px;padding:11px 14px;font-size:.82rem;color:#1e40af;margin-bottom:20px;line-height:1.5}
.drop-zone{border:2px dashed var(--b);border-radius:11px;padding:36px 18px;text-align:center;cursor:pointer;transition:.2s;background:var(--bg)}
...
.error{background:#fee2e2;border:1px solid #fca5a5;color:#dc2626;border-radius:7px;padding:10px 13px;font-size:.87rem;margin-top:12px;display:none}
...
<div class="card">
  ...
  <div class="tg-banner" id="tg-banner">
    📱 <strong>Telegram Bot:</strong> Send your <code>.json</code> or <code>.txt</code> file
    to the bot and receive the ready HTML quiz instantly.
  </div>
...
dz.addEventListener('drop',e=>{e.preventDefault();dz.classList.remove('over');
  const f=e.dataTransfer.files[0];if(f){const dt=new DataTransfer();dt.items.add(f);fi.files=dt.files;fn.textContent=f.name;sb.disabled=false;}});
...
async function tgSendMessage(token, chatId, text, parseMode = 'HTML') {
...
// ════════════════════════════════════════════════════════════
const WELCOME_MSG = `👋 <b>Welcome to Quiz Generator Bot!</b>

Send me a <code>.json</code> or <code>.txt</code> file containing quiz questions and I'll generate a fully interactive, bilingual (English + हिंदी) HTML quiz.

<b>Features in every generated quiz:</b>
✅ Bilingual EN + हिं on every question
🌙 Dark / Light mode toggle
🏅 Custom marking &amp; negative marking
📊 Performance stats &amp; session history
...
async function handleTelegram(request, token) {
  let update;
  ...
  if (text.startsWith('/start') || text.startsWith('/help')) {
    await tgSendMessage(token, chatId, WELCOME_MSG);
    return okResp();
  }
  ...
  if (doc) {
    ...
    if (!['json', 'txt'].includes(ext)) {
      await tgSendMessage(token, chatId, '❌ Please send a <b>.json</b> or <b>.txt</b> file.', 'HTML');
      return okResp();
    }

    await tgSendMessage(token, chatId, '⚙️ Processing your file…');

    const fileUrl = await tgGetFileUrl(token, doc.file_id);
    if (!fileUrl) {
      await tgSendMessage(token, chatId, '❌ Could not access your file. Please try again.');
      return okResp();
    }
    ...
    try { questions = JSON.parse(content); } catch {
      await tgSendMessage(token, chatId, '❌ Invalid JSON. Please check your file format.');
      return okResp();
    }

    if (!Array.isArray(questions) || !questions.length) {
      await tgSendMessage(token, chatId, '❌ JSON must be a non-empty array of question objects.');
      return okResp();
    }
    ...
    if (!result?.ok) {
      await tgSendMessage(token, chatId,
        `⚠️ Generated but could not send file: ${result?.description || 'unknown error'}`);
    }
    return okResp();
  }

  await tgSendMessage(token, chatId,
    '📄 Please send a <b>.json</b> or <b>.txt</b> quiz file. Type /help to see the expected format.', 'HTML');
  return okResp();
}
...
async function setupWebhook(request, token) {
  ...
  const result  = await tgApi(token, 'setWebhook', {
    url: webhook, allowed_updates: ['message', 'channel_post'],
  });
  return jsonResponse({
    ok:        result.ok,
    webhook,
    ...
    next_step: result.ok
      ? '✅ Bot is ready! Send a .json file to your bot in Telegram.'
      : '❌ Setup failed. Check your TELEGRAM_TOKEN in environment variables.',
  }, result.ok ? 200 : 500);
}
...
async function handleGenerate(request) {
  ...
  const filename = file.name || 'quiz.json';
...
export default {
  async fetch(request, env) {
    ...
    if (method === 'POST' && path === '/telegram')
      return token ? handleTelegram(request, token) : okResp();

    if (method === 'GET' && path === '/setup')
      return token
        ? setupWebhook(request, token)
        : jsonResponse({ error: 'TELEGRAM_TOKEN not configured in environment variables.' }, 400);
