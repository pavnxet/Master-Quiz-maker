#!/usr/bin/env python3
"""
Quiz HTML Generator — Bilingual + Merge + Advanced Features
------------------------------------------------------------
Usage:
    python3 quiz_generator.py <file1> [file2] [output.html]

    If two input files are given, their questions are merged into one quiz.
"""
...
def merge_questions(*lists):
    """Merge any number of question lists. Deduplicates by English/Hindi text."""
    seen, merged = set(), []
    for lst in lists:
        for q in lst:
            key = (q.get("qEnglish") or q.get("qHindi") or str(q)).strip().lower()
            if key not in seen:
                seen.add(key)
                merged.append(q)
    return merged
...
.h-date{{color:var(--muted);font-size:.75px}}
...
<nav>
  <div class="logo">
    <span>🎓</span>
    <span class="quiz-title-edit" id="quiz-title" contenteditable="true" title="Click to rename">{title_s}</span>
  </div>
  <div class="nav-tabs">
...
<div id="review-page" class="page">
  <div class="card">
    ...
    <div class="filter-row">
      <label>Subject</label><select id="review-subject" onchange="renderReview()"><option value="">All</option></select>
      <label>Show</label>
...
titleEl.addEventListener('input', () => {{ heroEl.textContent = titleEl.textContent.trim() || '{title_s}'; document.title = titleEl.textContent.trim() || '{title_s}'; }});
titleEl.addEventListener('blur', () => {{ if (!titleEl.textContent.trim()) titleEl.textContent = '{title_s}'; heroEl.textContent = titleEl.textContent.trim(); document.title = titleEl.textContent.trim(); }});
...
function renderQuestion() {{
  ...
  document.getElementById('quiz-meta-left').textContent='Q '+(s.currentIdx+1)+' / '+total+'  ·  Done: '+s.answers.filter(a=>a!==null).length;
  ...
  document.getElementById('q-english').innerHTML=q.qEnglish||'';
  document.getElementById('q-hindi').innerHTML=q.qHindi||'';
...
function finishQuiz(){{
  ...
  const pct=Math.round(correct/s.questions.length*100), elapsed=Math.round((Date.now()-s.startTime)/1000);
...
function reviewCard({{q,a,status}},n){{
  ...
  const optRows=optsEn.map((opt,i)=>{{
    ...
    return '<div class="review-option '+cls+'"><span style="font-weight:700;flex-shrink:0;width:22px">'+icon+String.fromCharCode(65+i)+'</span><span class="option-texts"><span class="opt-english">'+opt+'</span>'+(optsHi[i]?'<span class="opt-hindi">'+optsHi[i]+'</span>':'')+'</span></div>';
  }}).join('');
...
function renderStatsPage(){{
  ...
  document.getElementById('stats-grid').innerHTML='<div class="stat-card"><div class="value">'+history.length+'</div><div class="label">Sessions</div></div><div class="stat-card"><div class="value">'+totalQ+'</div><div class="label">Attempted</div></div><div class="stat-card"><div class="value">'+avgPct+'%</div><div class="label">Avg</div></div><div class="stat-card"><div class="value">'+best+'%</div><div class="label">Best</div></div><div class="stat-card"><div class="value">'+bestMk+'</div><div class="label">Best Marks</div></div>';
  const subAcc={{}};
  ...
  document.getElementById('stats-history').innerHTML=history.slice(0,20).map(r=>{{
    ...
    const color=r.pct>=80?'#16a34a':r.pct>=60?'#4f46e5':'#dc2626';
    return '<div class="history-item"><span class="h-score" style="color:'+color+'">'+r.pct+'%</span><span class="h-detail">'+r.correct+'/'+r.total+(r.totalScore!==undefined?' · '+r.totalScore+'/'+r.maxScore+' marks':'')+' · '+r.subject+'</span><span class="h-date">'+ds+'</span></div>';
  }}).join('');
}}
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 quiz_generator.py <file1> [file2] [output.html]")
        sys.exit(1)

    args = sys.argv[1:]
    input_files = [a for a in args if not a.endswith(".html")]
    output_file = next((a for a in args if a.endswith(".html")), None)

    if len(input_files) == 0:
        print("Error: no input file given"); sys.exit(1)

    questions = load_questions(input_files[0])
    title     = Path(input_files[0]).stem.replace("_", " ").title()

    if len(input_files) >= 2:
        q2 = load_questions(input_files[1])
        questions = merge_questions(questions, q2)
        t2 = Path(input_files[1]).stem.replace("_", " ").title()
        title = f"{title} + {t2}"
        print(f"Merged {len(questions)} unique questions from 2 files.")
    else:
        print(f"Loaded {len(questions)} questions.")

    if not output_file:
        output_file = str(Path(input_files[0]).parent / (Path(input_files[0]).stem + "_quiz.html"))
